...
def test(model, test_loader, criterion, device):
    ...
    print(f"Test Accuracy {correction/len(test_loader.dataset)*100}%\n")
...
def save_model(model, path_to_model):
    ## Save the trained model to 'SM_MODEL_DIR' /opt/ml/model
    print('Saving the model!!!')

    modeldir_path = f"{path_to_model}/modelp3.pth" #os.path.join(path_to_model, "modelp3.pth") ::::::!!!!!!!
    with open(modeldir_path, 'wb') as f:
        torch.save(model.state_dict(), f)
    
    # PyTorch requires the inference script to be in a folder called code inside of model directory
    inference_path = f"{path_to_model}/code/" #os.path.join(path_to_model, 'code/') ::::::!!!!!!!

    if not os.path.exists(inference_path):
        os.mkdir(inference_path)

    shutil.copy("inference.py", inference_path)

    print(f"Saving inference code to {inference_path}")
...
def model_fn(model_dir):
    """
    Load/deserialize the model and return it for 
    ...
    modeldir_path = f"{path_to_model}/modelp3.pth" #os.path.join(model_dir, "modelp3.pth") ::::::!!!!!!!
...
def input_fn(request_body, request_content_type):
    """
    Deseriaize request_body into a object that can be used for prediction
    request_body is the s3 path to image(s)
    """
    ...
    if request_content_type == "application/x-image":
        dataset = datasets.ImageFolder(root=request_body, transform=transform)
        return(torch.utils.data.DataLoader(dataset, batch_size=1))
        #return(create_data_loaders(request_body, 1))
...
def predict_fn(input_object, model):
    ...
    with torch.no_grad():
        for image, target in input_object:
            image = image.to(device)
            target = target.to(device)
            output = model(image)
